...
- **Justice** — exposer références, relations, divergences, convergences et indéterminations sous une forme publique minimisée.
- **Politiques publiques** — relier financements, dispositifs, opérateurs et indicateurs puis rendre visibles les recalculs reproductibles.
- **Nombres & information** — manipuler localement nombres premiers, logarithmes, modulo, logique booléenne, Shannon, SHA-256 et signature cryptographique.
...
- **Produits du quotidien** — partir de questions simples sur l’air et l’eau transportés, puis distinguer fonction, architecture, volume, masse, logistique, énergie et limites de comparaison.
...
Le module Énergie calcule localement dans le navigateur. Le module DPE interroge directement les jeux de données publics ADEME depuis le navigateur. La page Argent · temps · dette et la première version de Produits du quotidien sont statiques. L’infrastructure d’hébergement et les services externes consultés peuvent traiter les données techniques nécessaires à la connexion, notamment l’adresse IP.

## Déploiement

GitHub Pages publie exclusivement le répertoire `public/` via `.github/workflows/pages.yml`.

Le workflow de validation de la frontière publique vérifie notamment la présence de `public/information/index.html`, `public/everyday-products/index.html`, du laboratoire `public/number-information-lab/`, de la page `public/number-information-lab/argent/` et des actifs d’identité visuelle publics.
