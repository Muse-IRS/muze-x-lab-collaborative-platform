(() => {
  ...
  const descriptions=[['Technique actuelle','Canvas 2D · deux essaims cyan/violet · champ plein viewport · attracteurs de contour'],['Rendu','profondeur perceptive émergente · rassemblement dynamique lorsque les frontières entrent dans le viewport'],['Statut','R&D exploratoire — métaphore visuelle ≠ modèle scientifique']];
  ...
  const state={width:1,height:1,dpr:1,particles:[],pointer:{x:0,y:0,active:false,pressure:0},anchor:{element:anchorLink,active:false,x:0,y:0,radius:0,strength:0},boundaries:[],last:performance.now()};
  ...
  function rebuild(){state.particles=Array.from({length:particleCount()},(_,i)=>makeParticle(i))}
  ...
  function updateParticle(p,dt,now){const teamPhase=p.team===0?0:Math.PI,baseX=state.width*(.5+Math.sin(now*.000075+teamPhase)*.29),baseY=state.height*(.5+Math.cos(now*.000061+teamPhase*.73)*.25);let centerX=baseX,centerY=baseY,spring=.000025;if(state.anchor.active){const blend=state.anchor.strength,offset=(p.team===0?-.12:.12)*state.anchor.radius;centerX=baseX*(1-blend)+(state.anchor.x+offset)*blend;centerY=baseY*(1-blend)+(state.anchor.y+state.anchor.radius*.15)*blend;spring+=.00038*blend}let force={x:(centerX-p.x)*spring,y:(centerY-p.y)*spring};force.x+=Math.cos(now*.00021*p.drift+p.phase)*.0055*p.drift;force.y+=Math.sin(now*.00018*p.drift+p.phase)*.0055*p.drift;const fdx=p.x-state.width*.5,fdy=p.y-state.height*.5,fd=Math.hypot(fdx,fdy)+.001,spin=p.team===0?1:-1;force.x+=(-fdy/fd)*.0015*spin;force.y+=(fdx/fd)*.0015*spin;
    for(const b of state.boundaries)applyRectBoundary(p,b,force);
    if(state.anchor.active){const dx=state.anchor.x-p.x,dy=state.anchor.y-p.y,d=Math.hypot(dx,dy)+.01,nx=dx/d,ny=dy/d,boundary=state.anchor.radius*.78,outside=clamp((d-boundary)/Math.max(state.anchor.radius*1.8,1),0,1),gather=(.014+outside*.052)*state.anchor.strength*(.62+p.z*.58);force.x+=nx*gather;force.y+=ny*gather;if(d<state.anchor.radius*1.08){const localSpin=.0062*state.anchor.strength*spin;force.x+=-ny*localSpin;force.y+=nx*localSpin}}
  ...
  function frame(now){const elapsed=Math.min(34,now-state.last);state.last=now;const dt=reducedMotion?.35:clamp(elapsed/16.667,.4,2.05);updateFields();ctx.shadowBlur=0;ctx.globalCompositeOperation='source-over';ctx.fillStyle=reducedMotion?'rgba(4,10,22,.55)':'rgba(4,10,22,.18)';ctx.fillRect(0,0,state.width,state.height);ctx.globalCompositeOperation='lighter';for(const p of state.particles){updateParticle(p,dt,now);drawParticle(p)}ctx.globalCompositeOperation='source-over';ctx.shadowBlur=0;requestAnimationFrame(frame)}
